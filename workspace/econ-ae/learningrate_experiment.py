...
file_lut = {
    'train': 'train_econ',
    'hessian': '../common/measure_hessian',
    'gradient': 'gradient',
    'CKA': 'CKA',
...
def metric_command(learning_rate, bitwidth, metric='hessian'):
    if metric == 'train':
        ...
        command = f'python {file_lut[EXPERIMENT_TYPE]}.py --training-type {TRAINING_TYPE} --arch {MODEL}_{bitwidth}b --data-subset --subset 1.0 --train-bs {BATCH_SIZE} --test-bs {BATCH_SIZE} '
    elif metric == 'neural_eff':
        command = f'python ./code/{file_lut[EXPERIMENT_TYPE]}.py --arch {MODEL}_{bitwidth}b --early-stopping --data-path {DATAPATH} --train-bs {BATCH_SIZE} --test-bs {BATCH_SIZE} --checkpoint-folder ../checkpoint/different_knobs_subset_10/{SUBFOLDER}{learning_rate}/{TRAINING_TYPE}/{MODEL}_{bitwidth}b/autoencoder '
...
def metric_params(learning_rate, bitwidth, metric='hessian'):
    if metric == 'train':
        ...
        parameters = f'--train-or-test test --hessian-batch-size {BATCH_SIZE} --mini-hessian-batch-size {BATCH_SIZE} --checkpoint-folder ../checkpoint/different_knobs_subset_10/{SUBFOLDER}{learning_rate}/{TRAINING_TYPE}/{MODEL}_{bitwidth}b/autoencoder --early-stopping '
    elif metric == 'CKA':
...
if __name__ == '__main__':
    ...
    print(f'***** Preparing to write {EXPERIMENT_TYPE} experiment for {MODEL} model *****')
    write_experiemnt()
