...
def writing_experiments():
    ...
    TRAINING_PARAMETERS="--stop-epoch 25 --training-type bs_decay --lr-standard 0.01 --bs-standard 1024 --ignore-incomplete-batch --exp-start 0 --exp-num 3"
    HESSIAN_PARAMETERS=f"--experiment-type hessian --hessian-batch 1 --early-stop-checkpoint --training-type bs_decay"
    CKA_PARAMETERS="--experiment-type CKA --early-stop-checkpoint --mixup-CKA --mixup-alpha 16 --training-type bs_decay"
    DIST_PARAMETERS="--experiment-type dist --early-stop-checkpoint --training-type bs_decay"
    LOSS_ACC_PARAMETERS="--experiment-type loss_acc --early-stop-checkpoint --training-type bs_decay"
    ...
    for i in range(len(widths)):
        ...
        ARCH=f"{args.experiment_model}_{WIDTH}b"
        ...
        CURVE_PARAMETERS=f"--experiment-type curve --save-frequency=100 --num-points 5 --eval-epoch {EPOCH_NUM} --epochs {EPOCH_NUM} --early-stop-checkpoint --training-type bs_decay"
        
        ###################
        # Special attention
        ###################

        FILE_NAME_SUFFIX=f"{WIDTH}b_bs_decay"
        WIDTH_COMMANDS=f"--weight-precision {WIDTH} --bias-precision {WIDTH} --act-precision {ACT_WIDTH}"
        DATA_COMMANDS=f"--data-type subset --data-path ../../data/{args.experiment_model}"
        RANGE_COMMANDS=f"--load-range customize --load {SUBSET} --temperature-type bs --temperature-range coarse"
...
if __name__ == "__main__":
    ...
    print("Starting")    
    parser = argparse.ArgumentParser(description='Loss Landscape Experiment')
    
    parser.add_argument('--experiment-model', type=str, default=['JT'], choices=['JT', 'ECON', 'RN07'])
    parser.add_argument('--script-folder-name', type=str, default = 'width_bs_decay', help='experiment name')
    ...
    args.script_folder_name = args.script_folder_name+'_'+args.experiment_model
...
# lr_decay 
# python example_experiment.py --metrics train --experiment-model JT
