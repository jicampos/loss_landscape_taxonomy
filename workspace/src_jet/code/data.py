...
def get_loader(args):
    ...
    trainset = JetTaggingDataset(path="../../data/train", features=features, preprocess="standardize")
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.train_bs, shuffle=True, **kwargs)

    testset = JetTaggingDataset(path="../../data/test", features=features, preprocess="standardize")
