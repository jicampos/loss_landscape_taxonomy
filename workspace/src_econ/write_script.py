...
def get_curve_folder(args, home_dir, val, load_suffix):
        
    # curve_folder = os.path.join(home_dir, "checkpoint/different_knobs_{7}_{6}/{0}_{1}/{2}/{3}/".format(args.temperature_type, val, args.training_type, args.arch, exp_id_1, exp_id_2, load_suffix, args.folder_suffix))
    curve_folder = os.path.join(home_dir, "checkpoint/different_knobs_{5}_{4}/{0}_{1}/{2}/{3}/".format(args.temperature_type, val, args.training_type, args.arch, load_suffix, args.folder_suffix))
    ...
    for load, load_suffix in load_list:
        ...
        for val in temperature_list:
            ...
            if args.experiment_type == 'dist':
                ...
                command = f"python ./code/neural_eff.py --arch {args.arch}{command_suffix} " \
                          + f"--checkpoint-folder {ckpt_folder} --result-location {result_name} " \
                          + f"> >(tee -a {result_folder_name}/model_dist.log) 2> >(tee -a {result_folder_name}/model_dist.err >&2)" 
                          #+ f"1>{result_folder_name}/model_dist.log " \
                          #+ f"2>{result_folder_name}/model_dist.err"
            ...
            if args.experiment_type == 'hessian':
                ...
                for exp_id_1,exp_id_2 in exps:
                    ...
                    curve_folder = get_curve_folder(args, home_dir, val, load_suffix)
                ...
                if args.training_type == 'normal':
                    ...
                    command_suffix += f' --one-lr-decay --max_epochs 100 --save-best'
                ...
                for exp_id in exp_range:
                    ...
                    command = f"python code/train.py --train --data_file ../../data/nELinks5.npy --training-type {args.training_type} " \
                                + f"--arch {args.arch} --save_dir {ckpt_folder} --file-prefix exp_{exp_id} " \
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment-type', type=str, default = 'train', 
                        choices = ['train', 'CKA', 'curve', 'dist', 'hessian', 'loss_acc', 'neural_eff'], 
    ...
    parser.add_argument('--data_file', type=str, default='../../data/data.npy',
                        help='file with dataset')
